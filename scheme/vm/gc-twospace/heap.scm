  ...
  (let loop ((addr start))
	...
	(let* ((d (fetch addr))
	       (next (address+ addr
				  ...
				  (header-length-in-a-units d)))))
	  (cond ((not (header? d))
		 (if (check-lost "Heap-check: unexpected non-header." addr)
		     (loop next)
		     #f))
		((address< end next)
		 (if (check-lost "Heap-check: header too large." addr)
		     (loop next)
		     #f))
		((b-vector-header? d)
		 (loop next))
...
(define (check-stob-contents start end)
  (let loop ((addr start))
	...
	(let ((x (fetch addr)))
	  (cond ((header? x)
		 (if (check-lost-header "Heap-check: unexpected header." addr x)
		     (loop (address1+ addr))
		     #f))
		((or (not (stob? x))
		     (check-stob x))
...
(define (check-lost-header message addr h)
  (write-string message (current-error-port))
  (write-string " At Address: " (current-error-port))
  (write-integer (address->integer addr) (current-error-port))
  (newline (current-error-port))

  (write-string "type: " (current-error-port))
  (write-integer (header-type h) (current-error-port))
  (write-string " length: " (current-error-port))
  (write-integer (header-length-in-bytes h) (current-error-port))
  (newline (current-error-port))

  (set! *heap-errors-left* (- *heap-errors-left* 1))
  (> *heap-errors-left* 0))

(define (check-lost-bounds message addr addr1)
  (write-string message (current-error-port))
  (write-string " At Address: " (current-error-port))
  (write-integer (address->integer addr1) (current-error-port))
  (write-string " points to: " (current-error-port))
  (write-integer (address->integer addr) (current-error-port))
  (newline (current-error-port))
