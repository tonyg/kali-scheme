...
(define (start-command-levels resume-args context
  ...
  (let ((mode (or (and (pair? resume-args)
                       (equal? (car resume-args)
                               "run-script")
                       (if (and
                            (equal? (cadr resume-args) "exec")
                            (or
                             (not (pair? (cdddr resume-args)))
                             (equal? (cadddr resume-args) "--")))
                           'exec
                           'script))
                  'repl)))
        ...
        (lambda ()
          (let-fluids $command-level-thread? #t
            ...
            (lambda ()
              ...
              (start-thunk)
              (if (eq? mode 'exec)
                  (let ((exps (let loop ((port (make-string-input-port
                                                (caddr (focus-object))))
                                         (exps '()))
                                (let ((exp (read port)))
                                  (if (eof-object? exp)
                                      (reverse exps)
                                      (loop port (cons exp exps)))))))
                    ;; FIXME: this code is duplicated in command.scm
                    (for-each (lambda (exp)
