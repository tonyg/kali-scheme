...
(define-command-syntax 'run "<exp> [<select>]" "evaluate an expression"
  '(expression &opt select))

(define (run exp . select-option)
  (let ((select (if (null? select-option)
                    #t
                    (eval (car select-option) (environment-for-commands)))))
    (if select
