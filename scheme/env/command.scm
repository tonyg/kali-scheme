...
(define-script-handler "exec"
  (lambda (args)
    (with-srfi-22-error-handling
     (lambda ()
       (let loop ((port (make-string-input-port (car args))))
         (let ((exp (read port)))
           (if (not (eof-object? exp))
               (begin
                 (display "exp: ") (display exp) (newline)
                 (eval exp (user-command-environment))
                 (loop port)))))
       (let ((script-args (cdr args)))
         (load-script-into (car script-args) (interaction-environment))
         (if (package-lookup (interaction-environment) 'main)
             ((environment-ref (interaction-environment) 'main) script-args))
               0)))
     0))
