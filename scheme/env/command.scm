...
(define (real-command-loop)
  ;(let-fluids $note-undefined #f    ;useful    ; removed for Kali
  ;    (lambda ()	))			; removed for Kali
...
(define (execute-command command)
  (cond ((eof-object? command)
	   ...
	   ; DYNAMIC-WIND removed for Kali to keep the dynamic-point small.
; Begin Kali code
	   (call-with-values
	    (lambda ()
...
(define (greet-user info)
  (let ((port (command-output)))
    (display "Welcome to Scheme 48 (Kali) " port)	; Kali code
