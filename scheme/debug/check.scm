...
(config '(run 
(define-structure foo (export foo-tests)
  (open scheme test-suites
	;assembler
	byte-vectors
	ports
  ...
  (begin
    ...
    (define-test-suite foo-tests)

(define-test-case many-args foo-tests
  (check 
...
(define-test-cases foo-tests
  (*1 (check (* 1 2 3) => 6))
  (*2 (check (* 214760876 10) => 2147608760))
  (*3 (check (* 47123 46039) => 2169495797)))

(define-test-case internal-define foo-tests
  (check (let ()
	      (define a 5)
	      (define b 15)
	      (+ a b))
...
(define-test-case read-byte foo-tests
...
(define-test-case apply foo-tests
  (check (apply list 1 2 '(3 4)) => '(1 2 3 4)))

(define-test-case char<->integer foo-tests
  (check (integer->char (char->integer #\a)) => #\a))

(define-test-case number->string foo-tests
  (check (number->string 12) => "12")
  (check (number->string 12 8) => "14")
  (check (number->string -12) => "-12")
  (check (number->string -12 8) => "-14"))

(define-test-case string->number foo-tests
...
(define-test-case q foo-tests
  (let ((q (make-queue)))
    (enqueue! q 'a)
    (check (dequeue! q) => 'a)))

(define-test-case random foo-tests
  (check (<= 0 ((make-random 7))) => #t))

(define-test-case sort foo-tests
  (check (sort-list '(2 3 1 3) <) => '(1 2 3 3)))

(define-test-case bigbit foo-tests
  (check (arithmetic-shift 1 100) => (expt 2 100)))

(define-test-case cr/lf foo-tests
  (check
...
(define-test-case format foo-tests
  (check (format #f "x~s" '(1 2)) => "x(1 2)"))

(define-test-case fancy-input-ports foo-tests
...
(define-test-case byte-sink-input-ports foo-tests
...
(define-test-case fancy-output-ports foo-tests
  (let* ((r 0)
	 (c 0)
...
(define-test-case limit-output foo-tests
  (check
...
(define-test-case destructure foo-tests
  (check (destructure (((x (y) z) '(b (a) c))) y) => 'a))

(define-test-case array foo-tests
  (check (let ((a (make-array 'b 3 4)))
	   (array-set! a 'a 1 2)
	   (array-ref a 1 2))
	 => 'a))

(define-test-case delay foo-tests
...
(define-test-case receive foo-tests
  (check (receive stuff (values 'a 'b) stuff) => '(a b)))

(define-test-case dump foo-tests
  (let ((z '(a "b" 3 #t)))
...
(define-test-case placeholders foo-tests
  (let ((r 0)
	(l1 (make-lock))
	(l2 (make-lock))
	(ph (make-placeholder)))
...
(define-test-case get/put foo-tests
  (check (begin (put 'foo 'prop 'a)
		(get 'foo 'prop)) 
	 => 'a))

(define-test-case search-trees foo-tests
  ...
  (let ((type (make-mask-type 'color-mask
    ...
    (let ((m0 (list->mask type (list (color black)
      ...
      (check (map color-name (mask->list m0))
	     => '(black white purple))
      (check (mask->integer m1) => #o11)

      (check (show (mask-set m1)) => '(black maroon))
      (check (show (mask-set m1 (color black))) => '(black maroon))
      (check (show (mask-set m1 (color black) (color white))) => '(black white maroon))

      (check (show (mask-clear m1)) => '(black maroon))
      (check (show (mask-clear m1 (color black))) => '(maroon))
      (check (show (mask-clear m1 (color black) (color white))) => '(maroon))

      (check (show (mask-union m0 m0)) => '(black white purple))
      (check (show (mask-union m0 m1)) => '(black white purple maroon))
      (check (show (mask-union m2 m0)) => '(black white purple))

      (check (show (mask-intersection m0 m0)) => '(black white purple))
      (check (show (mask-intersection m0 m1)) => '(black))
      (check (show (mask-intersection m2 m0)) => '())

      (check (show (mask-subtract m0 m0)) => '())
      (check (show (mask-subtract m0 m1)) => '(white purple))
      (check (show (mask-subtract m0 m2)) => '(black white purple))
...
(define-test-case sockets foo-tests
  (check
...
(define-test-case float foo-tests
...
(user '(run (define-test-suite all-tests (bench-tests
					  test1-tests test2-tests test3-tests
					  foo-tests))))

(user '(run (run-test-suite all-tests)))
