...
;
; Locations have three slots, id, uid, and contents.


(define (transport-location loc)
  (let* ((data (allocate-d-vector (enum stob location) location-size #f))
         (descriptor (car data))
         (vector (cdr data)))
    (vector-set! vector location-uid-offset vm-false)	; Kali code
    (vector-set! vector
...
; Kali: symbols now have an id slot as well.

(define (transport-symbol symbol)
  (let* ((data (allocate-d-vector (enum stob symbol) symbol-size #t))
         (descriptor (car data))
         (vector (cdr data)))
    (vector-set! vector 0 vm-false)	; Kali code ; no uid yet
    (vector-set! vector
                 1			; Kali change, was 0
