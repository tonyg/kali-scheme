...
#include "scheme48/libscheme48/scheme48.h" /* $SCHEME48/c/scheme48.h */
#include <sys/time.h>
...
extract_state (s48_word_t * tmp,
               s48_arena_t arena,
               s48_word_t * state,
               int n)
{
  s48_new_vector_ref (tmp, arena, state, n);
  return (unsigned int)s48_new_extract_integer (arena, tmp);
}

void
s48_mrg32k3a_pack_state (s48_word_t * result,
                         s48_arena_t arena,
                         s48_word_t * state)
{
  struct s48_mrg32k3a_pack_state_frame
    {
      S48_FRAME;
      s48_word_t tmp;
    } l;
...
#define REF(i) extract_state (&l.tmp, arena, state, i)

  /* copy the numbers from state into s */
  s.x10 = (double)(REF( 0) + (REF( 1) << 16));
  s.x11 = (double)(REF( 2) + (REF( 3) << 16));
  s.x12 = (double)(REF( 4) + (REF( 5) << 16));
  s.x20 = (double)(REF( 6) + (REF( 7) << 16));
  s.x21 = (double)(REF( 8) + (REF( 9) << 16));
  s.x22 = (double)(REF(10) + (REF(11) << 16));

#undef REF

  S48_UNPROTECT_FRAME (l);

  s48_new_make_value (result, arena, &s, sizeof (s));
}
...
static void
enter_state (s48_word_t * tmp,
             s48_arena_t arena,
             s48_word_t * state,
             int n,
             unsigned short val)
{
  s48_fixnum_to_number (tmp, arena, val);
  s48_new_vector_set (arena, state, n, tmp);
}

void
s48_mrg32k3a_unpack_state (s48_word_t * result,
                           s48_arena_t arena,
                           s48_word_t * state)
{
  struct s48_mrg32k3a_unpack_state_frame
    {
      S48_FRAME;
      s48_word_t tmp;
    } l;
  ...
  s48_value_extract (arena, state, &s, sizeof (s));

  S48_PROTECT_FRAME (l);

  /* make and fill a Scheme vector with the numbers */
  s48_make_unspecific (&l.tmp, arena);
  s48_new_make_vector (result, arena, 12, &l.tmp);

#define SET(i, x) { \
  long x1 = (long)((x) / 65536.0); \
  long x0 = (long)((x) - 65536.0 * (double)x1); \
  enter_state (&l.tmp, arena, result, i    , x0); \
  enter_state (&l.tmp, arena, result, i + 1, x1); }
...
void
s48_mrg32k3a_random_range (s48_word_t * result,
                           s48_arena_t arena)
{
  s48_fixnum_to_number (result, arena, m_max);
}
...
void
s48_mrg32k3a_random_integer (s48_word_t * result,
                             s48_arena_t arena,
                             s48_word_t * state,
                             s48_word_t * range)
{
  ...
  s48_value_extract (arena, state, &s, sizeof (s));
  n = s48_new_extract_integer (arena, range);

  if ((n < 1) || (n > m_max))
    s48_new_raise_fixnum_range_error (arena, n, 1, m_max);
  ...
  /* check the range */
  if ((xq < 0.0) || (xq >= (double)m_max))
    s48_new_raise_fixnum_range_error (arena, xq, 0, m_max);

  /* return result */
  s48_value_set (arena, state, &s, sizeof (s));
  s48_fixnum_to_number (result, arena, (long)xq);
}
...
void
s48_mrg32k3a_random_real (s48_word_t * result,
                          s48_arena_t arena,
                          s48_word_t * state)
{
  state_t s;
  ...
  s48_make_double (result, arena, x);
}
...
/* Kludge for scsh */
static void
s48_current_time (s48_word_t * result,
                  s48_arena_t arena)
{
  struct timeval tv;

  gettimeofday (&tv, 0);
  s48_fixnum_to_number (result, arena, tv.tv_sec);
}
