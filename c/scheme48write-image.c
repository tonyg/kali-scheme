#include <stdio.h>
#include "prescheme.h"
#include "scheme48vm.h"
#include "scheme48heap.h"

long s48_write_image(long, long, FILE *);
static long Sundumpable_recordsS;
static long Sundumpable_countS;
static char * Sfrom_beginS;
static char * Sfrom_endS;
static char * Sto_beginS;
static char * Sto_endS;
static char * ShpS;
static long SstatusS;

long s48_write_image(long resume_proc_0X, long undumpables_1X, FILE * port_2X)
{
  char * arg1K1;
  char * arg1K0;
  long arg0K1;
  long arg0K0;
  char * merged_arg1K1;
  long merged_arg0K0;

  int real_copy_object_return_tag;
  long real_copy_object0_return_value;
  char * real_copy_object1_return_value;
  int gc_recordP_return_tag;
  char gc_recordP0_return_value;
  int note_undumpableB_return_tag;
  int trace_image_value_return_tag;
  long trace_image_value0_return_value;
  long thing_3X;
  char * frontier_4X;
  long x_5X;
  long thing_6X;
  long stob_7X;
  char * new_frontier_128X;
  long new_127X;
  char * data_addr_126X;
  long h_125X;
  long header_124X;
  char * addr_123X;
  long x_122X;
  long i_121X;
  long len_120X;
  long vector_119X;
  char * new_hp_118X;
  long new_thing_117X;
  char * a_116X;
  char v_115X;
  long type_114X;
  char v_113X;
  long stob_112X;
  long h_111X;
  long h_110X;
  long d_109X;
  char * addr_108X;
  long status_107X;
  long v_106X;
  long v_105X;
  long v_104X;
  long v_103X;
  long v_102X;
  long v_101X;
  long v_100X;
  char * v_99X;
  long v_98X;
  char * v_97X;
  long v_96X;
  long v_95X;
  long v_94X;
  long h_93X;
  long v_92X;
  long h_91X;
  long d_90X;
  char * addr_89X;
  char * addr_88X;
  long next_link_87X;
  long new_foo_86X;
  long v_85X;
  long resumer_records_84X;
  char * addr_83X;
  long next_link_82X;
  long new_foo_81X;
  char * a_80X;
  char * addr_79X;
  long foo_78X;
  long link_77X;
  char * addr_76X;
  long entry_75X;
  long type_74X;
  long record_73X;
  char * next_72X;
  long header_71X;
  char * addr_70X;
  char * a_69X;
  char * addr_68X;
  long foo_67X;
  long okay_link_66X;
  long foo_link_65X;
  long link_64X;
  char * begin_63X;
  char * data_start_62X;
  char * start_hp_61X;
  long i_60X;
  char * addr_59X;
  char * addr_58X;
  long okay_link_57X;
  long foo_link_56X;
  long foo_link_55X;
  long i_54X;
  long id_53X;
  long channel_52X;
  long foo_link_51X;
  long table_50X;
  long imported_bindings_49X;
  long i_48X;
  char * a_47X;
  long channel_46X;
  long table_45X;
  long table_44X;
  long i_43X;
  long x2_42X;
  long *channels_41X;
  long count_40X;
  char * a_39X;
  long contents_38X;
  long header_37X;
  char * addr_36X;
  char * frontier_35X;
  long new_thing_34X;
  char * a_33X;
  char * addr_32X;
  long next_link_31X;
  long new_foo_30X;
  char * addr_29X;
  long foo_28X;
  char v_27X;
  long type_26X;
  char v_25X;
  char * next_24X;
  long thing_23X;
  char * frontier_22X;
  char * addr_21X;
  long done_link_20X;
  long foo_link_19X;
  char * end_18X;
  char * start_17X;
  long foo_link_16X;
  long resume_proc_15X;
  long i_14X;
  long table_13X;
  long table_12X;
  char * v_11X;
  char * v_10X;
  char * v_9X;
  char * v_8X;
 {  v_8X = s48_heap_begin();
  Sfrom_beginS = v_8X;
  v_9X = s48_heap_pointer();
  Sfrom_endS = v_9X;
  v_10X = s48_oldspace_begin();
  Sto_beginS = v_10X;
  v_11X = s48_oldspace_end();
  Sto_endS = v_11X;
  ShpS = (Sto_beginS);
  Sundumpable_recordsS = undumpables_1X;
  Sundumpable_countS = 0;
  table_12X = s48_exported_bindings();
  merged_arg0K0 = table_12X;
  trace_image_value_return_tag = 0;
  goto trace_image_value;
 trace_image_value_return_0:
  table_13X = trace_image_value0_return_value;
  arg0K0 = 0;
  goto L3965;}
 L3965: {
  i_14X = arg0K0;
  if ((1024 == i_14X)) {
    merged_arg0K0 = resume_proc_0X;
    trace_image_value_return_tag = 1;
    goto trace_image_value;
   trace_image_value_return_1:
    resume_proc_15X = trace_image_value0_return_value;
    arg1K0 = (Sto_beginS);
    goto L3983;}
  else {
    foo_link_16X = *((long *) ((((char *) (-3 + table_13X))) + (((i_14X)<<2))));
    arg0K0 = foo_link_16X;
    arg0K1 = 1;
    goto L3398;}}
 L3983: {
  start_17X = arg1K0;
  end_18X = ShpS;
  arg1K0 = start_17X;
  arg1K1 = (ShpS);
  goto L2853;}
 L3398: {
  foo_link_19X = arg0K0;
  done_link_20X = arg0K1;
  if ((0 == (3 & foo_link_19X))) {
    arg0K0 = (3 + (-4 & foo_link_19X));
    goto L3400;}
  else {
    arg0K0 = foo_link_19X;
    goto L3400;}}
 L2853: {
  addr_21X = arg1K0;
  frontier_22X = arg1K1;
  if ((addr_21X < end_18X)) {
    thing_23X = *((long *) addr_21X);
    next_24X = addr_21X + 4;
    if ((2 == (3 & thing_23X))) {
      if ((2 == (3 & thing_23X))) {
        if (((31 & (((thing_23X)>>2))) < 17)) {
          goto L2874;}
        else {
          goto L2879;}}
      else {
        goto L2874;}}
    else {
      merged_arg0K0 = thing_23X;
      gc_recordP_return_tag = 0;
      goto gc_recordP;
     gc_recordP_return_0:
      v_25X = gc_recordP0_return_value;
      if (v_25X) {
        type_26X = *((long *) (((char *) (-3 + thing_23X))));
        merged_arg0K0 = type_26X;
        gc_recordP_return_tag = 1;
        goto gc_recordP;
       gc_recordP_return_1:
        v_27X = gc_recordP0_return_value;
        if (v_27X) {
          if ((1 == (*((long *) ((((char *) (-3 + type_26X))) + 4))))) {
            merged_arg0K0 = thing_23X;
            note_undumpableB_return_tag = 0;
            goto note_undumpableB;
           note_undumpableB_return_0:
            *((long *) addr_21X) = (*((long *) ((((char *) (-3 + thing_23X))) + 4)));
            arg1K0 = addr_21X;
            arg1K1 = frontier_22X;
            goto L2853;}
          else {
            goto L2895;}}
        else {
          goto L2895;}}
      else {
        goto L2895;}}}
  else {
    ShpS = frontier_22X;
    if (((ShpS) < (Sto_endS))) {
      if ((end_18X < (ShpS))) {
        arg1K0 = end_18X;
        goto L3983;}
      else {
        goto L2072;}}
    else {
      ps_error("Image writing error: ran out of space in new heap", 0);
      goto L2072;}}}
 L3400: {
  foo_28X = arg0K0;
  if ((1 == foo_28X)) {
    addr_29X = (((char *) (-3 + table_13X))) + (((i_14X)<<2));
    S48_WRITE_BARRIER(table_13X, addr_29X, done_link_20X);
    *((long *) addr_29X) = done_link_20X;
    arg0K0 = (1 + i_14X);
    goto L3965;}
  else {
    merged_arg0K0 = foo_28X;
    trace_image_value_return_tag = 2;
    goto trace_image_value;
   trace_image_value_return_2:
    new_foo_30X = trace_image_value0_return_value;
    next_link_31X = *((long *) ((((char *) (-3 + new_foo_30X))) + 12));
    addr_32X = (((char *) (-3 + new_foo_30X))) + 12;
    S48_WRITE_BARRIER(new_foo_30X, addr_32X, done_link_20X);
    *((long *) addr_32X) = done_link_20X;
    if ((3 == (3 & new_foo_30X))) {
      arg0K0 = next_link_31X;
      arg0K1 = (-4 & new_foo_30X);
      goto L3398;}
    else {
      arg0K0 = next_link_31X;
      arg0K1 = new_foo_30X;
      goto L3398;}}}
 L2874: {
  if ((1078 == thing_23X)) {
    goto L2879;}
  else {
    arg1K0 = next_24X;
    arg1K1 = frontier_22X;
    goto L2853;}}
 L2879: {
  arg1K0 = (next_24X + (-4 & (3 + ((long)(((unsigned long)thing_23X)>>8)))));
  arg1K1 = frontier_22X;
  goto L2853;}
 L2895: {
  if ((3 == (3 & thing_23X))) {
    a_33X = ((char *) (-3 + thing_23X));
    if ((a_33X < (Sfrom_beginS))) {
      arg1K0 = next_24X;
      arg1K1 = frontier_22X;
      goto L2853;}
    else {
      if ((a_33X < (Sfrom_endS))) {
        merged_arg0K0 = thing_23X;
        merged_arg1K1 = frontier_22X;
        real_copy_object_return_tag = 0;
        goto real_copy_object;
       real_copy_object_return_0:
        new_thing_34X = real_copy_object0_return_value;
        frontier_35X = real_copy_object1_return_value;
        *((long *) addr_21X) = new_thing_34X;
        arg1K0 = next_24X;
        arg1K1 = frontier_35X;
        goto L2853;}
      else {
        arg1K0 = next_24X;
        arg1K1 = frontier_22X;
        goto L2853;}}}
  else {
    arg1K0 = next_24X;
    arg1K1 = frontier_22X;
    goto L2853;}}
 L2072: {
  arg1K0 = (Sto_beginS);
  goto L2076;}
 L2076: {
  addr_36X = arg1K0;
  if ((addr_36X < (ShpS))) {
    header_37X = *((long *) addr_36X);
    if ((1078 == header_37X)) {
      contents_38X = *((long *) (addr_36X + 4));
      if ((3 == (3 & contents_38X))) {
        if ((3 == (3 & contents_38X))) {
          a_39X = ((char *) (-3 + contents_38X));
          if ((a_39X < (Sfrom_beginS))) {
            arg0K0 = 1;
            goto L2119;}
          else {
            if ((a_39X < (Sfrom_endS))) {
              if ((3 == (3 & (*((long *) ((((char *) (-3 + contents_38X))) + -4)))))) {
                arg0K0 = (*((long *) ((((char *) (-3 + contents_38X))) + -4)));
                goto L2119;}
              else {
                arg0K0 = 1;
                goto L2119;}}
            else {
              arg0K0 = 1;
              goto L2119;}}}
        else {
          arg0K0 = 1;
          goto L2119;}}
      else {
        goto L2127;}}
    else {
      goto L2127;}}
  else {
    count_40X = s48_channel_count();
    channels_41X = s48_channels();
    arg0K0 = 0;
    goto L3669;}}
 L2119: {
  x2_42X = arg0K0;
  *((long *) (addr_36X + 4)) = x2_42X;
  goto L2127;}
 L2127: {
  arg1K0 = (addr_36X + (4 + (-4 & (3 + ((long)(((unsigned long)header_37X)>>8))))));
  goto L2076;}
 L3669: {
  i_43X = arg0K0;
  if ((i_43X == count_40X)) {
    table_44X = s48_symbol_table();
    merged_arg0K0 = table_44X;
    trace_image_value_return_tag = 3;
    goto trace_image_value;
   trace_image_value_return_3:
    table_45X = trace_image_value0_return_value;
    arg0K0 = 0;
    goto L4008;}
  else {
    channel_46X = *(channels_41X + i_43X);
    if ((1 == channel_46X)) {
      goto L3697;}
    else {
      if ((3 == (3 & channel_46X))) {
        if ((3 == (3 & channel_46X))) {
          a_47X = ((char *) (-3 + channel_46X));
          if ((a_47X < (Sto_beginS))) {
            goto L3720;}
          else {
            if ((a_47X < (ShpS))) {
              goto L3690;}
            else {
              goto L3720;}}}
        else {
          goto L3720;}}
      else {
        goto L3690;}}}}
 L4008: {
  i_48X = arg0K0;
  if ((1024 == i_48X)) {
    imported_bindings_49X = s48_imported_bindings();
    merged_arg0K0 = imported_bindings_49X;
    trace_image_value_return_tag = 4;
    goto trace_image_value;
   trace_image_value_return_4:
    table_50X = trace_image_value0_return_value;
    arg0K0 = 0;
    goto L4029;}
  else {
    foo_link_51X = *((long *) ((((char *) (-3 + table_45X))) + (((i_48X)<<2))));
    arg0K0 = foo_link_51X;
    arg0K1 = 1;
    goto L3280;}}
 L3697: {
  arg0K0 = (1 + i_43X);
  goto L3669;}
 L3720: {
  if ((3 == (3 & (*((long *) ((((char *) (-3 + channel_46X))) + -4)))))) {
    goto L3690;}
  else {
    goto L3697;}}
 L3690: {
  merged_arg0K0 = channel_46X;
  trace_image_value_return_tag = 5;
  goto trace_image_value;
 trace_image_value_return_5:
  channel_52X = trace_image_value0_return_value;
  ps_write_string("Channel closed in dumped image: ", (stderr));
  id_53X = *((long *) ((((char *) (-3 + channel_52X))) + 4));
  if ((0 == (3 & id_53X))) {
    ps_write_integer((((id_53X)>>2)), (stderr));
    goto L1786;}
  else {
    ps_write_string((((char *)(((char *) (-3 + id_53X))))), (stderr));
    goto L1786;}}
 L4029: {
  i_54X = arg0K0;
  if ((1024 == i_54X)) {
    arg0K0 = 0;
    goto L2356;}
  else {
    foo_link_55X = *((long *) ((((char *) (-3 + table_50X))) + (((i_54X)<<2))));
    arg0K0 = foo_link_55X;
    arg0K1 = 1;
    goto L3161;}}
 L3280: {
  foo_link_56X = arg0K0;
  okay_link_57X = arg0K1;
  if ((0 == (3 & foo_link_56X))) {
    arg0K0 = (3 + (-4 & foo_link_56X));
    goto L3282;}
  else {
    arg0K0 = foo_link_56X;
    goto L3282;}}
 L1786: {
  { long ignoreXX;
  PS_WRITE_CHAR(10, (stderr), ignoreXX) }
  addr_58X = ((char *) (-3 + channel_52X));
  S48_WRITE_BARRIER(channel_52X, addr_58X, 0);
  *((long *) addr_58X) = 0;
  addr_59X = (((char *) (-3 + channel_52X))) + 8;
  S48_WRITE_BARRIER(channel_52X, addr_59X, -4);
  *((long *) addr_59X) = -4;
  goto L3697;}
 L2356: {
  i_60X = arg0K0;
  if ((1024 == i_60X)) {
    start_hp_61X = ShpS;
    data_start_62X = (ShpS) + 4;
    ShpS = data_start_62X;
    begin_63X = Sto_beginS;
    arg1K0 = begin_63X;
    goto L2470;}
  else {
    link_64X = *((long *) ((((char *) (-3 + table_50X))) + (((i_60X)<<2))));
    if ((0 == (3 & link_64X))) {
      arg0K0 = (3 + (-4 & link_64X));
      goto L2330;}
    else {
      arg0K0 = link_64X;
      goto L2330;}}}
 L3161: {
  foo_link_65X = arg0K0;
  okay_link_66X = arg0K1;
  if ((0 == (3 & foo_link_65X))) {
    arg0K0 = (3 + (-4 & foo_link_65X));
    goto L3163;}
  else {
    arg0K0 = foo_link_65X;
    goto L3163;}}
 L3282: {
  foo_67X = arg0K0;
  if ((1 == foo_67X)) {
    addr_68X = (((char *) (-3 + table_45X))) + (((i_48X)<<2));
    S48_WRITE_BARRIER(table_45X, addr_68X, okay_link_57X);
    *((long *) addr_68X) = okay_link_57X;
    arg0K0 = (1 + i_48X);
    goto L4008;}
  else {
    if ((3 == (3 & foo_67X))) {
      if ((3 == (3 & foo_67X))) {
        a_69X = ((char *) (-3 + foo_67X));
        if ((a_69X < (Sto_beginS))) {
          goto L3323;}
        else {
          if ((a_69X < (ShpS))) {
            goto L3291;}
          else {
            goto L3323;}}}
      else {
        goto L3323;}}
    else {
      goto L3291;}}}
 L2470: {
  addr_70X = arg1K0;
  if ((addr_70X < start_hp_61X)) {
    header_71X = *((long *) addr_70X);
    next_72X = addr_70X + (4 + (-4 & (3 + ((long)(((unsigned long)header_71X)>>8)))));
    if ((2 == (3 & header_71X))) {
      if ((9 == (31 & (((header_71X)>>2))))) {
        record_73X = 3 + (((long) (addr_70X + 4)));
        type_74X = *((long *) (((char *) (-3 + record_73X))));
        if ((3 == (3 & type_74X))) {
          if ((9 == (31 & ((((*((long *) ((((char *) (-3 + type_74X))) + -4))))>>2))))) {
            if ((3 == (3 & (*((long *) ((((char *) (-3 + type_74X))) + 4)))))) {
              if ((4 < ((Sto_endS) - (ShpS)))) {
                *((long *) (ShpS)) = record_73X;
                ShpS = ((ShpS) + 4);
                arg1K0 = next_72X;
                goto L2470;}
              else {
                goto L4060;}}
            else {
              arg1K0 = next_72X;
              goto L2470;}}
          else {
            arg1K0 = next_72X;
            goto L2470;}}
        else {
          arg1K0 = next_72X;
          goto L2470;}}
      else {
        arg1K0 = next_72X;
        goto L2470;}}
    else {
      ps_write_string("new image is not in a consistent state", (stderr));
      goto L4060;}}
  else {
    *((long *) start_hp_61X) = (10 + (((((ShpS) - data_start_62X))<<8)));
    arg0K0 = (3 + (((long) data_start_62X)));
    goto L3921;}}
 L2330: {
  entry_75X = arg0K0;
  if ((1 == entry_75X)) {
    arg0K0 = (1 + i_60X);
    goto L2356;}
  else {
    addr_76X = (((char *) (-3 + entry_75X))) + 8;
    S48_WRITE_BARRIER(entry_75X, addr_76X, 529);
    *((long *) addr_76X) = 529;
    link_77X = *((long *) ((((char *) (-3 + entry_75X))) + 12));
    if ((0 == (3 & link_77X))) {
      arg0K0 = (3 + (-4 & link_77X));
      goto L2330;}
    else {
      arg0K0 = link_77X;
      goto L2330;}}}
 L3163: {
  foo_78X = arg0K0;
  if ((1 == foo_78X)) {
    addr_79X = (((char *) (-3 + table_50X))) + (((i_54X)<<2));
    S48_WRITE_BARRIER(table_50X, addr_79X, okay_link_66X);
    *((long *) addr_79X) = okay_link_66X;
    arg0K0 = (1 + i_54X);
    goto L4029;}
  else {
    if ((3 == (3 & foo_78X))) {
      if ((3 == (3 & foo_78X))) {
        a_80X = ((char *) (-3 + foo_78X));
        if ((a_80X < (Sto_beginS))) {
          goto L3204;}
        else {
          if ((a_80X < (ShpS))) {
            goto L3172;}
          else {
            goto L3204;}}}
      else {
        goto L3204;}}
    else {
      goto L3172;}}}
 L3323: {
  if ((3 == (3 & (*((long *) ((((char *) (-3 + foo_67X))) + -4)))))) {
    goto L3291;}
  else {
    arg0K0 = (*((long *) ((((char *) (-3 + foo_67X))) + 4)));
    arg0K1 = okay_link_57X;
    goto L3280;}}
 L3291: {
  merged_arg0K0 = foo_67X;
  trace_image_value_return_tag = 6;
  goto trace_image_value;
 trace_image_value_return_6:
  new_foo_81X = trace_image_value0_return_value;
  next_link_82X = *((long *) ((((char *) (-3 + new_foo_81X))) + 4));
  addr_83X = (((char *) (-3 + new_foo_81X))) + 4;
  S48_WRITE_BARRIER(new_foo_81X, addr_83X, okay_link_57X);
  *((long *) addr_83X) = okay_link_57X;
  if ((3 == (3 & new_foo_81X))) {
    arg0K0 = next_link_82X;
    arg0K1 = (-4 & new_foo_81X);
    goto L3280;}
  else {
    arg0K0 = next_link_82X;
    arg0K1 = new_foo_81X;
    goto L3280;}}
 L4060: {
  ShpS = start_hp_61X;
  arg0K0 = 1;
  goto L3921;}
 L3921: {
  resumer_records_84X = arg0K0;
  if ((1 == resumer_records_84X)) {
    arg1K0 = (Sfrom_beginS);
    goto L4074;}
  else {
    SstatusS = NO_ERRORS;
    if (((SstatusS) == NO_ERRORS)) {
      PS_WRITE_CHAR(10, port_2X, v_85X)
      SstatusS = v_85X;
      goto L1121;}
    else {
      goto L1121;}}}
 L3204: {
  if ((3 == (3 & (*((long *) ((((char *) (-3 + foo_78X))) + -4)))))) {
    goto L3172;}
  else {
    arg0K0 = (*((long *) ((((char *) (-3 + foo_78X))) + 12)));
    arg0K1 = okay_link_66X;
    goto L3161;}}
 L3172: {
  merged_arg0K0 = foo_78X;
  trace_image_value_return_tag = 7;
  goto trace_image_value;
 trace_image_value_return_7:
  new_foo_86X = trace_image_value0_return_value;
  next_link_87X = *((long *) ((((char *) (-3 + new_foo_86X))) + 12));
  addr_88X = (((char *) (-3 + new_foo_86X))) + 12;
  S48_WRITE_BARRIER(new_foo_86X, addr_88X, okay_link_66X);
  *((long *) addr_88X) = okay_link_66X;
  if ((3 == (3 & new_foo_86X))) {
    arg0K0 = next_link_87X;
    arg0K1 = (-4 & new_foo_86X);
    goto L3161;}
  else {
    arg0K0 = next_link_87X;
    arg0K1 = new_foo_86X;
    goto L3161;}}
 L4074: {
  addr_89X = arg1K0;
  if ((addr_89X < (Sfrom_endS))) {
    d_90X = *((long *) addr_89X);
    if ((2 == (3 & d_90X))) {
      arg0K0 = d_90X;
      goto L4080;}
    else {
      h_91X = *((long *) ((((char *) (-3 + d_90X))) + -4));
      *((long *) addr_89X) = h_91X;
      arg0K0 = h_91X;
      goto L4080;}}
  else {
    return ENOMEM;}}
 L1121: {
  if (((SstatusS) == NO_ERRORS)) {
    PS_WRITE_CHAR(12, port_2X, v_92X)
    SstatusS = v_92X;
    goto L1130;}
  else {
    goto L1130;}}
 L4080: {
  h_93X = arg0K0;
  arg1K0 = (addr_89X + (4 + (-4 & (3 + ((long)(((unsigned long)h_93X)>>8))))));
  goto L4074;}
 L1130: {
  if (((SstatusS) == NO_ERRORS)) {
    PS_WRITE_CHAR(10, port_2X, v_94X)
    SstatusS = v_94X;
    goto L1139;}
  else {
    goto L1139;}}
 L1139: {
  if (((SstatusS) == NO_ERRORS)) {
    SstatusS = (ps_write_string("Vanilla 22", port_2X));
    goto L1146;}
  else {
    goto L1146;}}
 L1146: {
  if (((SstatusS) == NO_ERRORS)) {
    PS_WRITE_CHAR(10, port_2X, v_95X)
    SstatusS = v_95X;
    goto L1155;}
  else {
    goto L1155;}}
 L1155: {
  if (((SstatusS) == NO_ERRORS)) {
    SstatusS = (ps_write_integer(4, port_2X));
    goto L1225;}
  else {
    goto L1225;}}
 L1225: {
  if (((SstatusS) == NO_ERRORS)) {
    PS_WRITE_CHAR(10, port_2X, v_96X)
    SstatusS = v_96X;
    goto L1157;}
  else {
    goto L1157;}}
 L1157: {
  v_97X = Sto_beginS;
  if (((SstatusS) == NO_ERRORS)) {
    SstatusS = (ps_write_integer(((((((long) v_97X)))>>2)), port_2X));
    goto L1245;}
  else {
    goto L1245;}}
 L1245: {
  if (((SstatusS) == NO_ERRORS)) {
    PS_WRITE_CHAR(10, port_2X, v_98X)
    SstatusS = v_98X;
    goto L1163;}
  else {
    goto L1163;}}
 L1163: {
  v_99X = ShpS;
  if (((SstatusS) == NO_ERRORS)) {
    SstatusS = (ps_write_integer(((((((long) v_99X)))>>2)), port_2X));
    goto L1265;}
  else {
    goto L1265;}}
 L1265: {
  if (((SstatusS) == NO_ERRORS)) {
    PS_WRITE_CHAR(10, port_2X, v_100X)
    SstatusS = v_100X;
    goto L1169;}
  else {
    goto L1169;}}
 L1169: {
  if (((SstatusS) == NO_ERRORS)) {
    SstatusS = (ps_write_integer(table_45X, port_2X));
    goto L1280;}
  else {
    goto L1280;}}
 L1280: {
  if (((SstatusS) == NO_ERRORS)) {
    PS_WRITE_CHAR(10, port_2X, v_101X)
    SstatusS = v_101X;
    goto L1171;}
  else {
    goto L1171;}}
 L1171: {
  if (((SstatusS) == NO_ERRORS)) {
    SstatusS = (ps_write_integer(table_50X, port_2X));
    goto L1295;}
  else {
    goto L1295;}}
 L1295: {
  if (((SstatusS) == NO_ERRORS)) {
    PS_WRITE_CHAR(10, port_2X, v_102X)
    SstatusS = v_102X;
    goto L1173;}
  else {
    goto L1173;}}
 L1173: {
  if (((SstatusS) == NO_ERRORS)) {
    SstatusS = (ps_write_integer(table_13X, port_2X));
    goto L1310;}
  else {
    goto L1310;}}
 L1310: {
  if (((SstatusS) == NO_ERRORS)) {
    PS_WRITE_CHAR(10, port_2X, v_103X)
    SstatusS = v_103X;
    goto L1175;}
  else {
    goto L1175;}}
 L1175: {
  if (((SstatusS) == NO_ERRORS)) {
    SstatusS = (ps_write_integer(resumer_records_84X, port_2X));
    goto L1325;}
  else {
    goto L1325;}}
 L1325: {
  if (((SstatusS) == NO_ERRORS)) {
    PS_WRITE_CHAR(10, port_2X, v_104X)
    SstatusS = v_104X;
    goto L1177;}
  else {
    goto L1177;}}
 L1177: {
  if (((SstatusS) == NO_ERRORS)) {
    SstatusS = (ps_write_integer(resume_proc_15X, port_2X));
    goto L1340;}
  else {
    goto L1340;}}
 L1340: {
  if (((SstatusS) == NO_ERRORS)) {
    PS_WRITE_CHAR(10, port_2X, v_105X)
    SstatusS = v_105X;
    goto L1179;}
  else {
    goto L1179;}}
 L1179: {
  if (((SstatusS) == NO_ERRORS)) {
    PS_WRITE_CHAR(12, port_2X, v_106X)
    SstatusS = v_106X;
    goto L1188;}
  else {
    goto L1188;}}
 L1188: {
  *((long *) (ShpS)) = 1;
  if (((SstatusS) == NO_ERRORS)) {
    SstatusS = (ps_write_block(port_2X, ((char *) (ShpS)), (((ShpS) + 4) - (ShpS))));
    goto L1207;}
  else {
    goto L1207;}}
 L1207: {
  if (((SstatusS) == NO_ERRORS)) {
    SstatusS = (ps_write_block(port_2X, ((char *) (Sto_beginS)), ((ShpS) - (Sto_beginS))));
    goto L1220;}
  else {
    goto L1220;}}
 L1220: {
  status_107X = SstatusS;
  arg1K0 = (Sfrom_beginS);
  goto L4096;}
 L4096: {
  addr_108X = arg1K0;
  if ((addr_108X < (Sfrom_endS))) {
    d_109X = *((long *) addr_108X);
    if ((2 == (3 & d_109X))) {
      arg0K0 = d_109X;
      goto L4102;}
    else {
      h_110X = *((long *) ((((char *) (-3 + d_109X))) + -4));
      *((long *) addr_108X) = h_110X;
      arg0K0 = h_110X;
      goto L4102;}}
  else {
    return status_107X;}}
 L4102: {
  h_111X = arg0K0;
  arg1K0 = (addr_108X + (4 + (-4 & (3 + ((long)(((unsigned long)h_111X)>>8))))));
  goto L4096;}
 trace_image_value: {
  stob_7X = merged_arg0K0;{
  arg0K0 = stob_112X;
  goto L4195;}
 L4195: {
  stob_112X = arg0K0;
  merged_arg0K0 = stob_7X;
  gc_recordP_return_tag = 2;
  goto gc_recordP;
 gc_recordP_return_2:
  v_113X = gc_recordP0_return_value;
  if (v_113X) {
    type_114X = *((long *) (((char *) (-3 + stob_7X))));
    merged_arg0K0 = type_114X;
    gc_recordP_return_tag = 3;
    goto gc_recordP;
   gc_recordP_return_3:
    v_115X = gc_recordP0_return_value;
    if (v_115X) {
      if ((1 == (*((long *) ((((char *) (-3 + type_114X))) + 4))))) {
        merged_arg0K0 = stob_7X;
        note_undumpableB_return_tag = 1;
        goto note_undumpableB;
       note_undumpableB_return_1:
        arg0K0 = (*((long *) ((((char *) (-3 + stob_7X))) + 4)));
        goto L4195;}
      else {
        goto L3802;}}
    else {
      goto L3802;}}
  else {
    goto L3802;}}
 L3802: {
  if ((3 == (3 & stob_7X))) {
    a_116X = ((char *) (-3 + stob_7X));
    if ((a_116X < (Sfrom_beginS))) {
      trace_image_value0_return_value = stob_7X;
      goto trace_image_value_return;}
    else {
      if ((a_116X < (Sfrom_endS))) {
        merged_arg0K0 = stob_7X;
        merged_arg1K1 = (ShpS);
        real_copy_object_return_tag = 1;
        goto real_copy_object;
       real_copy_object_return_1:
        new_thing_117X = real_copy_object0_return_value;
        new_hp_118X = real_copy_object1_return_value;
        ShpS = new_hp_118X;
        trace_image_value0_return_value = new_thing_117X;
        goto trace_image_value_return;}
      else {
        trace_image_value0_return_value = stob_7X;
        goto trace_image_value_return;}}}
  else {
    trace_image_value0_return_value = stob_7X;
    goto trace_image_value_return;}}
 trace_image_value_return:
  switch (trace_image_value_return_tag) {
  case 0: goto trace_image_value_return_0;
  case 1: goto trace_image_value_return_1;
  case 2: goto trace_image_value_return_2;
  case 3: goto trace_image_value_return_3;
  case 4: goto trace_image_value_return_4;
  case 5: goto trace_image_value_return_5;
  case 6: goto trace_image_value_return_6;
  default: goto trace_image_value_return_7;
  }}

 note_undumpableB: {
  thing_6X = merged_arg0K0;{
  if (((Sundumpable_countS) < ((((3 + ((long)(((unsigned long)(*((long *) ((((char *) (-3 + (Sundumpable_recordsS)))) + -4))))>>8))))>>2)))) {
    vector_119X = Sundumpable_recordsS;
    len_120X = (((3 + ((long)(((unsigned long)(*((long *) ((((char *) (-3 + vector_119X))) + -4))))>>8))))>>2);
    arg0K0 = 0;
    goto L2728;}
  else {
    goto note_undumpableB_return;}}
 L2728: {
  i_121X = arg0K0;
  if ((i_121X == len_120X)) {
    x_122X = Sundumpable_recordsS;
    addr_123X = (((char *) (-3 + x_122X))) + ((((Sundumpable_countS))<<2));
    S48_WRITE_BARRIER(x_122X, addr_123X, thing_6X);
    *((long *) addr_123X) = thing_6X;
    Sundumpable_countS = (1 + (Sundumpable_countS));
    goto note_undumpableB_return;}
  else {
    if (((*((long *) ((((char *) (-3 + vector_119X))) + (((i_121X)<<2))))) == thing_6X)) {
      goto note_undumpableB_return;}
    else {
      arg0K0 = (1 + i_121X);
      goto L2728;}}}
 note_undumpableB_return:
  switch (note_undumpableB_return_tag) {
  case 0: goto note_undumpableB_return_0;
  default: goto note_undumpableB_return_1;
  }}

 gc_recordP: {
  x_5X = merged_arg0K0;{
  if ((3 == (3 & x_5X))) {
    header_124X = *((long *) ((((char *) (-3 + x_5X))) + -4));
    if ((3 == (3 & header_124X))) {
      if ((3 == (3 & header_124X))) {
        gc_recordP0_return_value = (9 == (31 & ((((*((long *) ((((char *) (-3 + header_124X))) + -4))))>>2))));
        goto gc_recordP_return;}
      else {
        gc_recordP0_return_value = 0;
        goto gc_recordP_return;}}
    else {
      if ((3 == (3 & x_5X))) {
        gc_recordP0_return_value = (9 == (31 & ((((*((long *) ((((char *) (-3 + x_5X))) + -4))))>>2))));
        goto gc_recordP_return;}
      else {
        gc_recordP0_return_value = 0;
        goto gc_recordP_return;}}}
  else {
    gc_recordP0_return_value = 0;
    goto gc_recordP_return;}}
 gc_recordP_return:
  switch (gc_recordP_return_tag) {
  case 0: goto gc_recordP_return_0;
  case 1: goto gc_recordP_return_1;
  case 2: goto gc_recordP_return_2;
  default: goto gc_recordP_return_3;
  }}

 real_copy_object: {
  thing_3X = merged_arg0K0;
  frontier_4X = merged_arg1K1;{
  h_125X = *((long *) ((((char *) (-3 + thing_3X))) + -4));
  if ((3 == (3 & h_125X))) {
    real_copy_object0_return_value = h_125X;
    real_copy_object1_return_value = frontier_4X;
    goto real_copy_object_return;}
  else {
    data_addr_126X = frontier_4X + 4;
    new_127X = 3 + (((long) data_addr_126X));
    new_frontier_128X = data_addr_126X + (-4 & (3 + ((long)(((unsigned long)h_125X)>>8))));
    if ((new_frontier_128X < (Sto_endS))) {
      goto L2001;}
    else {
      ps_error("ran out of memory while writing image", 0);
      goto L2001;}}}
 L2001: {
  *((long *) ((((char *) (-3 + thing_3X))) + -4)) = new_127X;
  *((long *) frontier_4X) = h_125X;
  memcpy((void *)data_addr_126X, (void *)(((char *) (-3 + thing_3X))),((long)(((unsigned long)h_125X)>>8)));
  real_copy_object0_return_value = new_127X;
  real_copy_object1_return_value = new_frontier_128X;
  goto real_copy_object_return;}
 real_copy_object_return:
  switch (real_copy_object_return_tag) {
  case 0: goto real_copy_object_return_0;
  default: goto real_copy_object_return_1;
  }}

}void
s48_write_image_init(void)
{
SstatusS = NO_ERRORS;
}
