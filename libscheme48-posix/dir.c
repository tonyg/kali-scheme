...
#include "scheme48/libscheme48/scheme48vm.h"		/* ps_close_fd() */
...
static s48_error_t
new_posix_opendir (s48_word_t * result,
                   s48_arena_t arena,
                   s48_word_t * svname)
{
  char * c_name;
  ...
  s48_string_lock_data (&c_name, arena, svname);
  RETRY_OR_RAISE_NULL (dp, opendir (c_name));
  s48_string_unlock_data (arena, c_name);

  *result = S48_MAKE_VALUE (DIR *);
  S48_UNSAFE_EXTRACT_VALUE (*result, DIR *) = dp;
...
static s48_error_t
new_posix_closedir (s48_word_t * result,
                    s48_arena_t arena,
                    s48_word_t * svdir)
{
  *result = posix_closedir (*svdir);
  return 0;
}
 ...
 */
static s48_error_t
new_posix_readdir (s48_word_t * result,
                   s48_arena_t arena,
                   s48_word_t * svdir)
{
  *result = posix_readdir (*svdir);
  return 0;
}
...
static s48_error_t
new_posix_working_directory (s48_word_t * result,
                             s48_arena_t arena,
                             s48_word_t * new_wd)
{
  *result = posix_working_directory (*new_wd);
  return 0;
}
...
static s48_error_t
new_posix_open (s48_word_t * result,
                s48_arena_t arena,
                s48_word_t * path,
                s48_word_t * options,
                s48_word_t * mode)
{
  *result = posix_open (*path, *options, *mode);
  return 0;
}
...
static s48_error_t
new_posix_file_stuff (s48_word_t * result,
                      s48_arena_t arena,
                      s48_word_t * op,
                      s48_word_t * arg0,
                      s48_word_t * arg1)
{
  *result = posix_file_stuff (*op, *arg0, *arg1);
  return 0;
}
...
static s48_value enter_time(time_t time);

static s48_error_t
new_enter_time (s48_word_t * result,
                s48_arena_t arena,
                time_t time)
{
  *result = enter_time (time);
  return 0;
}
...
static time_t extract_time(s48_value time);

static s48_error_t
new_extract_time (time_t * result,
                  s48_arena_t arena,
                  s48_word_t * value)
{
  *result = extract_time (*value);
}
...
static s48_error_t
new_posix_ctime (s48_word_t * result,
                 s48_arena_t arena,
                 s48_word_t * sch_time)
{
  *result = posix_ctime (*sch_time);
  return 0;
}
...
static s48_error_t
new_posix_time (s48_word_t * result,
                s48_arena_t arena)
{
  *result = posix_time ();
}
...
s48_error_t
s48_new_enter_mode (s48_word_t * result,
                s48_arena_t arena,
                mode_t mode)
{
  *result = enter_mode (mode);
  return 0;
}
...
s48_error_t
s48_new_extract_mode (mode_t * result,
                      s48_arena_t arena,
                      s48_word_t * sch_mode)
{
  *result = s48_extract_mode (*sch_mode);
  return 0;
}
...
static s48_error_t
new_posix_file_info (s48_word_t * result,
                     s48_arena_t arena,
                     s48_word_t * svname,
                     s48_word_t * follow_link_p,
                     s48_word_t * mode_enum)
{
  *result = posix_file_info (*svname, *follow_link_p, *mode_enum);
  return 0;
}
