...
(define-structures ((environments environments-interface))
  (open scheme-level-2
...
(define-module (make-scheme environments evaluation)
  (define-structures ((scheme scheme-interface))
    (open scheme-level-2
	  environments
	  evaluation))
  scheme)
...
(define-module (make-mini-command scheme) ;copied from debug-packages.scm
  (define-structures ((mini-command (export command-processor)))
    (open scheme
...
(define-module (make-initial-system scheme command)
  ...
  (define-structures ((initial-system (export start)))
    (open scheme
	  command
...
(define-structures ((ensures-loaded  ;Cf. (link-initial-system) and Makefile
...
(define-structures ((for-reification for-reification-interface))
