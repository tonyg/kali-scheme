...
\cvar{name} \etc)}.  However, in place of
a name one may write {\code (\cvar{name} \cvar{type})}, indicating the type
of \cvar{name}'s binding.  Currently the type field is ignored, except
...
\qquad
   \syn{definition} \=\goesto{}~
      \tt(d\=efine-module (\syn{name} \arbno{(\syn{name} \syn{interface})}) \\
...
\item \code{,load \cvar{filespec} \etc} \\
...
The environment's tower is used at compile time for
 ...
 \code{(define-syntax \cvar{name} \cvar{macro-source})},
 \code{(let-syntax ((\cvar{name} \cvar{macro-source}) \ldots) \cvar{body})},
 and so forth.
