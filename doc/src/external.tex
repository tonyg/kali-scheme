...
\noindent{}The values returned by \code{s48\_extract\_string} and
 \code{s48\_extract\_byte\_vector} point to the actual
 storage used by the string or byte vector; they are valid only until the next
 \link{garbage collection}[; see Section~\ref{gc}]{gc}.
