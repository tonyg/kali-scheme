...
\begin{protos}
\proto{fork}{}{process-id or \code{\#f}}
...
\end{protos}
\noindent
A \cvar{process-id} is a boxed integer.
...
\begin{protos}
\proto{process-id-exit-status}{ process-id}{integer or \code{\#f}}
\proto{process-id-terminating-signal}{ process-id}{signal or \code{\#f}}
...
\begin{protos}
\proto{name->signal}{ symbol}{signal or \code{\#f}}
\proto{integer->signal}{ integer}{signal}
\proto{signal?}{ x}{boolean}
\proto{signal-name}{ signal}{symbol or \code{\#f}}
...
\proto{maybe-dequeue-signal!}{ queue-queue}{signal or \code{\#f}}
\end{protos}
\noindent
...
\begin{protos}
\proto{lookup-environment-variable}{ string}{string or \code{\#f}}
...
\proto{read-directory-stream}{ directory}{name or \code{\#f}}
...
Care should be exercised when using output ports which do not have
 the nonblocking option set.
Ordinary Scheme~48 output ports have the nonblocking option set to
 prevent write operations from causing the Scheme process to block.
...
\code{Rename} moves the file pointed to by \cvar{old-path} to the
 location pointed to by \cvar{new-path}.
...
These may not be the masks actually used by the underlying OS).
...
\proto{port->fd}{ port}{\#f or integer}
\end{protos}
\noindent
...
\begin{protos}
\proto{is-a-terminal?}{ port}{boolean}
\proto{terminal-name}{ port}{string}
\end{protos}
\noindent
\code{Is-a-terminal?} returns true if \cvar{port} has an underlying
 file descriptor that is associated with a terminal.
For such ports \code{terminal-name} returns the name of the
...
\code{exec[l|v][e|p|$\epsilon$]}
 &\code{exec, exec-with-environment, exec-file,} \\
 &\code{\ \ exec-file-with-environment, exec-with-alias}\\
...
\code{getenv}&\code{lookup-environment-variable, environment-alist}\\
...
\code{uname}&\code{os-name, os-node-name, os-release-name,}\\
 &\code{\ \ os-version-name, machine-name}\\
