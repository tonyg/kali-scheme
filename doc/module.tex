...
\begin{tabbing}
   ...
   \syn{definition} \=\goesto{}~
      \tt(define-package (\arbno{(\syn{name} \syn{signature})})
          \arbno{\syn{clause}}) \\
	\>\altz{}~ \tt(define-signature \syn{name} \syn{signature}) \\
   ...
   \syn{signature} \=\goesto{}~ \tt(export \arbno{\syn{item}}) \\
	\>\altz{}~ \syn{name} \\
	\>\altz{}~ \tt(compound-signature \arbno{\syn{signature}}) \\
...
The {\tt compound-signature} operator forms a signature that is the
...
\qquad
   \syn{definition} \=\goesto{}~
      \tt(d\=\tt{}efine-module (\syn{name} \arbno{(\syn{name} \syn{signature})}) \\
...
\item
\begin{code}
,load-into \var{struct-name} \var{file} \etc
\end{code}
...
The need to make structures and signatures second-class instead of
